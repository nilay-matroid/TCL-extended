...
def evaluation(model, data_loader, tokenizer, device, config, only_text2image=False):
    ...
    for i in range(0, num_text, text_bs):
        ...
        text_embed = F.normalize(model.text_proj(text_feat[:,0,:])).cpu()
...
def main(args, config):
    ...
    train_dataset, val_dataset, test_dataset = create_dataset('personreid_re', config)  
    ...
    for epoch in range(0, max_epoch):
        ...
        if epoch % args.val_freq == 0 or epoch == max_epoch - 1 or args.evaluate:
            ...
            score_val_i2t, score_val_t2i = evaluation(model_without_ddp, val_loader, tokenizer, device, config, only_text2image=args.only_text2image)
    
        if utils.is_main_process() and (epoch % args.val_freq == 0 or epoch == max_epoch - 1 or args.evaluate):  
            val_result = itm_eval(score_val_i2t, score_val_t2i, val_loader.dataset.txt2id, val_loader.dataset.identities, only_text2image=args.only_text2image)
            print(val_result)
            test_result = itm_eval(score_test_i2t, score_test_t2i, test_loader.dataset.txt2id, test_loader.dataset.identities, only_text2image=args.only_text2image)    
            print(test_result)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val_freq', type=int, default=2, help="Frequency to eval during training")
