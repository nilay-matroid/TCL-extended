...
from dataset.caption_dataset import re_eval_dataset
...
class Visualizer:
    ...
    def search_texts(self, texts: List[str]) -> List[List[int]]:
        num_text = len(texts)
        text_bs = self.config['batch_size_test']  # 256
        text_feats = []
        text_embeds = []
        text_atts = []
        ...
        for i in tqdm(range(0, num_text, text_bs)):
            ...
            text_input = self.tokenizer(text, padding='max_length', truncation=True, max_length=30, return_tensors="pt").to(self.args.device)
            
            text_output = self.model.text_encoder(text_input.input_ids, attention_mask = text_input.attention_mask, mode='text')  
            text_feat = text_output.last_hidden_state
            text_embed = F.normalize(self.model.text_proj(text_feat[:, 0, :])).cpu()
